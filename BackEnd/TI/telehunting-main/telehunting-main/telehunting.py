...
async def scrape_messages(client, entity, message_limit, keywords, channel_manager, affiliated_channel=None):
    ...
    try:
        entity_name = await get_entity_name(entity)
        download_dir = f"downloads/{entity_name.replace(' ', '_')}"
        ...
        async for message in client.iter_messages(entity, limit=message_limit):
            ...
            # Save attached files (documents like .txt, .csv, .zip)
            if message.document:
                filename = message.file.name or f"{message.id}_{message.date}.bin"
                if filename.lower().endswith(('.txt', '.csv', '.zip', '.log', '.json')):
                    try:
                        file_path = os.path.join(download_dir, filename)
                        await client.download_media(message, file_path)
                        print_success(f"Downloaded file: {file_path}")
                    except Exception as e:
                        print_error(f"Failed to download file {filename}: {e}")
...
if __name__ == "__main__":
    ...
    API_ID = "21873991"
    API_HASH = "3a34f06f4f86d03a89b8d4345a70e27e"
    PHONE_NUMBER = "+994553688116"
